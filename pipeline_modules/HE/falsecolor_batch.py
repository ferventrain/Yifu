...
def resolve_color_settings(color_key: str, nuclei_hue: str) -> dict[str, list[float]]:
    ...
    if color_key.upper() == "IHC":
        return settings
    preset = HE_COLOR_PRESETS.get(nuclei_hue, HE_COLOR_PRESETS["default"])
...
def safe_background_levels(
    image: np.ndarray,
    *,
    threshold: float | int,
) -> tuple[float, float]:
    """
    Wrap falsecolor.getBackgroundLevels for empty / near-empty planes.

    The library indexes the 95th percentile of foreground pixels and crashes
    when no pixel exceeds ``threshold`` (common on stack-edge blank slices).
    """
    img = np.asarray(image, dtype=np.float64)
    if img.size == 0:
        return 0.0, 0.0
    if not np.any(img > float(threshold)):
        mid = float(np.median(img))
        return mid, mid
    try:
        midrange, background = falsecolor.getBackgroundLevels(img, threshold=threshold)
        return float(midrange), float(background)
...
def build_flat_field_2d(
    ...
    *,
    ...
    midrange, background = safe_background_levels(img, threshold=bg_threshold)
...
def subtract_asymmetric_background(
    ...
    *,
    ...
    if nuc_factor > 0:
        _, bkg_nuc = safe_background_levels(nuclei_f, threshold=nuc_threshold)
        nuclei_f = np.clip(nuclei_f - float(nuc_factor) * bkg_nuc, 0, 65535)
    if cyto_factor > 0:
        _, bkg_cyto = safe_background_levels(cyto_f, threshold=cyto_threshold)
...
def render_falsecolor(
    ...
    *,
    ...
    flatfield_nuc_scale: float = 0.7,
    flatfield_cyto_scale: float = 2.6,
) -> np.ndarray:
...
def _process_one_slice(task: dict) -> dict:
    ...
    if task.get("he_preprocess", True):
        pp = HePreprocessParams()
        pp.deconv.backend = task.get("deconv_backend", "rl")
        pp.deconv.sigma = float(task.get("deconv_sigma", 1.2))
        pp.deconv.iterations = int(task.get("deconv_iterations", 10))
        pp.destripe.backend = task.get("destripe_backend", "fft")
        pp.destripe.strength = float(task.get("destripe_strength", 0.85))
        pp.destripe.notch_width = float(task.get("destripe_notch_width", 2.0))
        pp.destripe.keep_fraction = float(task.get("destripe_keep_fraction", 0.04))
        pp.destripe.sigma = float(task.get("destripe_smooth_sigma", 40.0))
        pp.destripe.orientation = task.get("destripe_orientation", "horizontal")
        pp.rolling_ball.radius = int(task.get("rolling_ball_radius", 5))
        pp.lcn.clip_limit = float(task.get("lcn_clip_limit", 2.0))
        pp.lcn.grid_size = int(task.get("lcn_grid_size", 16))
        cyto_pre = preprocess_cyto(cyto_raw, pp)
        nuc_pack = preprocess_nuclei(
            nuclei_raw,
            use_lcn=bool(task.get("use_lcn", False)),
            params=pp,
        )
        nuclei_for_color = nuc_pack["nuclei"]
        cyto_for_color = cyto_pre
    else:
        nuclei_for_color = nuclei_raw
        cyto_for_color = cyto_raw
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=16,
        help=(
            "Parallel worker processes (default: 16). Most HE preprocess time is "
            "CPU-bound (RL deconv / FFT destripe / rolling ball); raising workers "
            "usually improves throughput even with --backend gpu. On large CPUs "
            "pair with OMP_NUM_THREADS=1-2 per process. Watch VRAM/RAM if you go "
            "much above 16-24 on one GPU."
        ),
    )
    parser.add_argument(
        "--color_key",
    ...
    parser.add_argument(
        ...
        default="default",
        help=(
            "HE palette bias (default: package HE). "
            "purple ≈ cyto rgba(245,179,249) + nuclei rgba(129,4,170); "
            "blue = cooler hematoxylin."
        ),
    )
    parser.add_argument(
        "--nuc_threshold",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default: on. Use --no-flatfield to disable."
        ),
    )
    parser.add_argument(
        "--flatfield_tile_size",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.7,
        help="Nuclei flat-field beta (default: 0.7).",
    )
    parser.add_argument(
        "--flatfield_cyto_scale",
        type=float,
        default=2.6,
        help="Cyto flat-field beta (default: 2.6).",
    )
    parser.add_argument(
        "--sharpen_alpha",
        type=float,
        ...
        help=(
            ...
            "default: 0.05). Higher values make nuclei look rounder/thicker."
        ),
    )
    parser.add_argument(
        "--bg_nuc_factor",
        ...
        help=(
            ...
            "(default: 3.0, strong nuclei background suppression)."
        ),
    )
    parser.add_argument(
        "--bg_cyto_factor",
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Run HE preprocess (deconv->destripe->rolling_ball) before falsecolor (default: on).",
    )
    parser.add_argument("--use_lcn", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--deconv_backend", choices=("rl", "placeholder", "identity"), default="rl")
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default: none (keep white H&E background). "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.backend == "gpu" and args.workers > 1:
        print(
            f"GPU backend with --workers {args.workers}: CPU preprocess runs in "
            "parallel; falsecolor kernels share one GPU (watch VRAM).",
            file=sys.stderr,
        )
        ...
        tasks.append(
            {
                ...
                "he_preprocess": args.he_preprocess,
                "use_lcn": args.use_lcn,
                "deconv_backend": args.deconv_backend,
                "deconv_sigma": args.deconv_sigma,
                "deconv_iterations": args.deconv_iterations,
                "destripe_backend": args.destripe_backend,
                "destripe_strength": args.destripe_strength,
                "destripe_notch_width": args.destripe_notch_width,
                "destripe_keep_fraction": args.destripe_keep_fraction,
                "destripe_smooth_sigma": args.destripe_smooth_sigma,
                "destripe_orientation": args.destripe_orientation,
                "rolling_ball_radius": args.rolling_ball_radius,
                "lcn_clip_limit": args.lcn_clip_limit,
                "lcn_grid_size": args.lcn_grid_size,
    ...
    print(
        ...
        f"he_preprocess={args.he_preprocess}, rolling_ball_radius={args.rolling_ball_radius}, "
    ...
    if args.workers <= 1:
        for task in tasks:
            try:
                ...
                failed += 1
                print(
                    f"[FAIL] {task['prefix']}: {type(exc).__name__}: {exc}",
                    file=sys.stderr,
                )
                continue
            completed += 1
        ...
        with ProcessPoolExecutor(max_workers=args.workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    failed += 1
                    print(
                        f"[FAIL] {prefix}: {type(exc).__name__}: {exc}",
                        file=sys.stderr,
                    )
                    continue
                completed += 1
    ...
    print(
        f"Done. Processed {completed} slices, failed {failed} "
        f"in {elapsed / 60:.1f} min."
    )
    return 1 if failed and completed == 0 else 0
