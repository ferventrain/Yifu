...
def compute_block_qc_metrics(
    *,
    ...
    metrics = []
    for block in _iter_blocks(shape, chunks):
...
def export_block_previews(
    ...
    *,
    ...
    for record in records:
        ...
        sample_preview_dir = preview_path / str(record.get("sample_id") or "sample")
        sample_preview_dir.mkdir(parents=True, exist_ok=True)
        block_id = str(record.get("block_id") or "block")
        chunk_index = str(record.get("chunk_index") or "0.0.0").replace(".", "-")
        image_preview_path = sample_preview_dir / f"{block_id}_chunk_{chunk_index}_image.tiff"
        mask_preview_path = sample_preview_dir / f"{block_id}_chunk_{chunk_index}_mask.tiff"
