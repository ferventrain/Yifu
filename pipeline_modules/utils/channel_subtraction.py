...
def subtract_worker(cx_file, c0_file, output_path, compression='lzw', global_weight=1.0):
    ...
    If shapes don't match, performs center-aligned cropping.
    """
    try:
        # Load images
        img_cx = tifffile.imread(str(cx_file))
        ...
        # If shapes don't match, center crop both to the smaller size
        if img_cx.shape != img_c0.shape:
            ...
            # Center crop: calculate starting positions
            start_h1 = (h1 - target_h) // 2
            start_w1 = (w1 - target_w) // 2
            start_h2 = (h2 - target_h) // 2
            start_w2 = (w2 - target_w) // 2
            
            img_cx = img_cx[start_h1:start_h1+target_h, start_w1:start_w1+target_w]
            img_c0 = img_c0[start_h2:start_h2+target_h, start_w2:start_w2+target_w]
...
def process_channel_subtraction(root_path, max_workers=None, compression='lzw', weight=1.0, adaptive=False, save_plots=False, 
                               sample_ratio=0.005, min_samples=10, max_samples=50, background_channel='ch0', output_path=None):
    root = Path(root_path)
    if not root.is_dir():
    ...
    if output_path is None:
        output_root = root  # Default: save in same folder as input
    else:
    ...
    # Try to parse filenames, if fails use sorted order
    bg_files = {}
    bg_sorted = sorted(bg_dir.glob("*.tif*"))
    for f in bg_sorted:
        _, z_idx = parse_filename(f.name)
        if z_idx:
            bg_files[z_idx] = f
    
    use_sorted_matching = len(bg_files) == 0
    if use_sorted_matching:
        print(f"Warning: Could not parse filenames, falling back to sorted order matching")
        print(f"Found {len(bg_sorted)} reference files in {bg_dir.name} (sorted mode)")
    else:
    ...
    # Traverse other folders (excluding bg channel and output folders)
    for folder in root.iterdir():
        if not folder.is_dir() or folder == bg_dir or folder.name.endswith("_subtracted"):
            continue
        
        target_subtracted_dir = output_root / f"{folder.name}_subtracted"
        ...
        cx_files = sorted(folder.glob("*.tif*"))
        if not cx_files:
            print(f"No TIFF files found in {folder.name}, skipping.")
            continue
        
        # Match files: either by z-index parsing or by sorted order
        if use_sorted_matching:
            # Fallback: use sorted order matching
            n_match = min(len(cx_files), len(bg_sorted))
            matched_pairs = list(zip(cx_files[:n_match], bg_sorted[:n_match]))
            print(f"  Using sorted order matching: {len(matched_pairs)} pairs")
        else:
            # Normal mode: match by z-index
            matched_pairs = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default=None, help="Output directory (default: save in same folder as input)")
    ...
    process_channel_subtraction(args.path, max_workers=args.workers, compression=comp, 
                               ...
                               background_channel=args.bg_channel, output_path=args.output)
