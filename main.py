...
def main():
    ...
    if args.test:
        ...
        cmd = "python src/modules/segmentation/cellpose_segmentation.py --test"
    ...
    if not args.skip_preprocessing:
        ...
        if not (reg_downsample_dir / "volume.nii.gz").exists():
            ...
            if res_cfg.exists():
                ...
                # Fallback to manual 10x downsample if no config (Adjust as needed!)
                print("Warning: resolution.json not found, using default 10x downsampling (0.1, 0.1, 0.1)")
                cmd = f"python src/modules/preprocessing/downsample.py --input_folder \"{sample_dir}/ch{reg_ch}\" --factor \"0.1,0.1,0.1\""
            
            run_command(cmd, "Step 1.2: Downsample Registration Channel")
            
        # 1.3 Downsample Target Channel (Optional, maybe needed for visualization or some checks)
    ...
    if not args.skip_registration:
        ...
        atlas_lbl = atlas_dir / "annotation.tiff" # or annotation_25.tif
    ...
    if not args.skip_segmentation:
        ...
        if not mask_tiff_dir.exists():
            # We can use the export function in cellpose_distributed or a separate tool
            # Let's assume cellpose_distributed has --output_tiff which runs export
            # Or we run a specific export command. 
            # Re-running cellpose script just for export is inefficient if segmentation is done.
            # Let's append --output_tiff to the command above OR run a helper.
            # For simplicity, let's assume the user ran it with export or we add a specific export step here.
            
            # Helper script to export (inline logic or call existing)
            # Re-using cellpose_distributed.py's export function via CLI if possible, 
            # but usually better to have a dedicated `zarr_to_tiff.py`.
            # For now, let's assume we run cellpose_distributed again but check if zarr exists inside it? 
            # No, let's use a simple python one-liner or assume it was done.
            pass
    ...
    if not args.skip_analysis:
        ...
        warped_label_dir = sample_dir / f"ch{target_ch}_atlas_label_upsampled"
        
        # Check if we need to downsample the mask?
        # analyze_density.py typically runs on full res or downsampled res?
        ...
        if not mask_tiff_dir.exists():
             print(f"Error: Mask TIFF folder not found at {mask_tiff_dir}. Segmentation export missing?")
             # Try to export from Zarr if missing
