...
class TestPipelineError:
    ...
    def test_exit_code_runtime(self):
        err = PipelineError(ErrorCode.CUDA_OOM, "oops")
...
class TestBuildRunManifest:
    ...
    def test_required_keys(self):
        m = self._build()
        for key in ("schema_version", "module", "entrypoint", "started_at", "ended_at", "duration_seconds"):
    ...
    def test_outputs_list(self, tmp_path):
        ...
        assert m["outputs"][0]["type"] == "file"
        assert m["outputs"][0]["size_bytes"] > 0

    def test_nonexistent_output_flagged(self, tmp_path):
        m = self._build(outputs=[tmp_path / "ghost.csv"])
        assert m["outputs"][0]["type"] == "missing"
