"""Unattended cFos run for YF2026070102 sham (AHYK_nao_sham) without the TIFF layer.
...
Launched automatically by _queue_YF2026070102_sham.py once the SICM run
finishes.
"""
...
REPO = Path(r"S:\Yifu")
SAMPLE_DIR = Path(r"S:\Arivis_Analysis\YF2026070102_sham")
...
IMS_PATH = Path(
    ...
    r"\20260724_09_16_41_YF2026070102_AHYK_nao_sham_Destripe_DONE"
    r"\YF2026070102_AHYK_nao_sham.ims"
)
...
MASKED_IMS = SAMPLE_DIR / "YF2026070102_AHYK_nao_sham_cfos_masked.ims"
...
def attach_to_harness() -> None:
    try:
        ...
        record = store.attach_external(
            SAMPLE_DIR,
            title="YF2026070102 sham cFos: ims->zarr->unet->masked ims",
            pid=os.getpid(),
            log_path=LOG_PATH,
...
def main() -> int:
    ...
    log(f"=== YF2026070102 sham cFos (no-TIFF flow) start pid={os.getpid()} ===")
